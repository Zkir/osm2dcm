...
public class ValidatorTask {
  ...
  public void execute(MpData mpData, TaskInfo taskInfo) throws IOException, MPParseException
  {
    //Основные параметры карты
    String strReportFileName; //Файл с отчетом
    String strViewPoint; //Начальная точка карты

    strReportFileName=taskInfo.parameters.get("reportfile");
    strViewPoint=taskInfo.parameters.get("viewpoint");

    System.out.println( "");
    System.out.println( "Validator/Postprocessor task has been started");
    System.out.println( "Report file: " + strReportFileName);
    System.out.println( "Viewpoint: " + strViewPoint);
    ProcessMP (mpData, strViewPoint,strReportFileName);
    System.out.println( "Validator/Postprocessor task has been finished OK");
    System.out.println( "Time used: "+ Long.toString((dtProcessEnd.getTime()-dtProcessStart.getTime())/1000)+ " s" );

  };


  private void ProcessMP (MpData mpData, String strViewPoint, String strReportFileName) throws IOException, MPParseException
  {

...
function PrintSummaryItem($xml){
    ...
    PrintSummaryItem1($xml,$strPubDate, "Россия");
	
	echo "<h2>Заграница</h2>";	
	PrintSummaryItem1($xml,$strPubDate,"Заграница");	
    echo ']]>';
    echo '</description>
        </item>';
}	

function PrintSummaryItem1($xml,$strPubDate,$strGroup){
	...
	foreach ($xml->map as $item)
    {
       if( (($strGroup=="Россия") and (substr($item->code,0,2)=='RU') and ($item->code!='RU-OVRV')) 
       	   or (($strGroup!="Россия") and (substr($item->code,0,2)!='RU'))
       	  )
       {
	...
	$xml_stat = simplexml_load_file("http://peirce.gis-lab.ru/statistics.xml");
	$intQAIndex3=0;
	$intQAIndex3Max=0;
	$intQAIndex3Min=100000;
	$intDateDiff=0;
	$intNumberOfMapsB=0;
	$MapID3Max='';
	$MapID3Min='';
    ...
    {
        if( (($strGroup=="Россия") and (substr($item->MapId,0,2)=='RU') and ($item->MapId!='RU-OVRV')) 
       	   or (($strGroup!="Россия") and (substr($item->MapId,0,2)!='RU'))
       	  )
       {
       	   ...
       	   $intQAIndex3=$intQAIndex3+$intDateDiff;
       	     if($intDateDiff>$intQAIndex3Max){
       	       ...
       	       $MapID3Max=$item->MapId;
       	       echo "$MapID3Max,  $intDateDiff  <br />";
       	     }  
       	     if($intDateDiff<$intQAIndex3Min){
       	       $intQAIndex3Min=$intDateDiff;
       	     $MapID3Min=$item->MapId;
       	     }  
       }	   
    }
	...
	echo '<p>Количество карт в группе "'.$strGroup.'" (за все время): '.$intNumberOfMaps.'</p>';
	echo '<p>Количество карт прошедших QA в группе "'.$strGroup.'" (за все время): '.$intNumberOfQAPassedMaps.'</p>';
	
	echo '<p>Показатель латентности I: '.number_format($intQAIndex1,1,'.', ' ') .' дней (Максимальный: '.$intQAIndex1Max.'  дней)</p>';
	echo '<p>Показатель латентности II: '.number_format($intQAIndex2,1,'.', ' ').' дней (Максимальный: '.$intQAIndex2Max.'  ('.$MapID2Max.'))</p>';
	echo '<p>Показатель латентности III: '.number_format($intQAIndex3,1,'.', ' ').' дней (Минимальный: '.number_format($intQAIndex3Min,1,'.', ' ').' ('.$MapID3Min.') -- Максимальный: '.number_format($intQAIndex3Max,1,'.', ' ').'  ('.$MapID3Max.'))</p>';

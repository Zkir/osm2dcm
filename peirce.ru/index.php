...
require_once("include/misc_utils.php"); 
  ...
  $zPage->title=_("Карты для Ситигид 10 - 12.x");
  $zPage->header=_("Карты для Ситигид 10 - 12.x");
  ...
  $zPage->WriteHtml( "<H1>"._("Карты для Ситигид 10.x - 12.x")."</H1>");
  $zPage->WriteHtml('<P> <img src="/img/peirce.jpg" height="65px"  style="float:left;"
  	                 ...
  	                 '._('На этой странице представленны карты Openstreetmap для навигационной программы Ситигид версии 10.x - 12.x (формат cgnet).').'
  	                 <P>Для вопросов по картам, запросов на отсутствующие в этом списке карты, багрепорты можно использовать <a href="http://forum.openstreetmap.org/viewtopic.php?id=7043">форуме OSM</a>,
  	                 писать на kontur282@gmail.com, или в <a href="https://t.me/cgmaps">телеграмм-канал </a></P> <P> </P>
                         Замечания и предложения по работе самой программы лучше всего отправлять на <a href="https://play.google.com/store/apps/details?id=cityguide.probki.net">гугл-плей</a>/эппл аппстор.</P>
  	             <P> </P>');
  
  $zPage->WriteHtml('<P>Карты для предыдущей версии СГ 7.8 (в формате cgmap) текущей конвертации находятся <a href="https://yadi.sk/d/LfG5SRRZtcXT5">здесь</a>.</P>');
//
  ...
  foreach ($xml->map as $item)
    {
        ...
        )
      {
        ...
        $zPage->WriteHtml( '<td><a href="'.$item->url.'">скачать</a></td> </tr>');
        //$zPage->WriteHtml( '<td><a href="http://peirce.osm.rambler.ru/static/cg8_maps/'.$item->code.'-OSM.cgnet"> скачать</a></td> </tr>');

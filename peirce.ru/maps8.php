...
require_once("include/misc_utils.php"); 
  ...
  $zPage->title=_("Карты для Ситигид 8.x");
  $zPage->header=_("Карты для Ситигид 8.x");

  $xml = simplexml_load_file("maplist_8.xml"); //Интерпретирует XML-файл в объект
  $xml3 = simplexml_load_file("maplist_old.xml"); //Ручные карты
  	  
  $zPage->WriteHtml( "<H1>"._("Карты для Ситигид 8.x")."</H1>");
  $zPage->WriteHtml('<P> <img src="/img/peirce.jpg" height="65px"  style="float:left;"
  	                 ...
  	                 '._('На этой странице представленны карты Openstreetmap для навигационной программы Ситигид 8.x.').' 
...
//  PrintMapListOld ($xml3,"Карты с пробками");
  $group=$_GET['group'];
 $zPage->WriteHtml('<H2>'._('Карты в формате cgnet').'</H2>');
 $zPage->WriteHtml(_('Карты для данной версии обновляться не будут.').'<br/>');

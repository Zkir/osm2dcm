...
function PrintQASummaryPage()
{
  global $zPage;
    ...
    '<div style="float:right">
	   <a href="?setlang=en">ENG</a> | <a href="?setlang=ru">RUS</a>  
	</div>');
  ...
  $lan =  file ("lan//".$lan_code."//001.lan");
  
  
      
	  $zPage->WriteHtml( '<h1>'.$lan[0].'</h1>');
      $zPage->WriteHtml( '<p>'.$lan[1]. //На этой странице представлены основные показатели, отражающие качество карт, при конвертации в СитиГид.
       	                       $lan[2]. //Проверяется адресный реестр, дорожный граф и отрисовка карты.
      	                       $lan[3]. //Данные обновляются одновременно с картами для СГ, т.е. по возможности ежедневно.
      	                  '</p>' );
      $zPage->WriteHtml( '<h2>'.$lan[4].' </h2>');//Дорожный граф  
      $zPage->WriteHtml( '<p>'.$lan[5]. //Проверяется связность дорожного графа, т.е. отсутствие фрагментов, оторванных от основной дорожной сети 
                               $lan[6]. // (такие фрагменты недоступны для рутинга.
                          '<a href="/blog/14435">'.$lan[7].'</a>'. //Подробнее про связность дорожного графа...
                          '</p> '.$lan[8]. //Также проверяется отсутствие дубликатов дорог.
                          ' <a href="/blog/16019">'.$lan[9].'</a></p>');//Подробнее дубликаты дорог...
    ...
    */
      ...
      $zPage->WriteHtml( '<h2>'.$lan[12].'</h2>'); //Адресный реестр

      $zPage->WriteHtml('<p> '.$lan[13].'</p>'.// Этот тест  показывает, какие дома/адреса <b>не</b> попадают в адресный поиск после конвертации карт в СитиГид.
                       '<p>'.$lan[14].'<br />'.$lan[15].'<br />'.$lan[16].'</p>');
      $zPage->WriteHtml( '<h2>'.$lan[17].'</h2>');//Отрисовка карты 
      $zPage->WriteHtml( '<p>'.$lan[18].'</p>');                   
      $zPage->WriteHtml( '<h2>'.$lan[19].'</h2>');//Система рейтинга
      $zPage->WriteHtml( '<p>'.$lan[20].'</p>
      	                  <p>'.$lan[21].'</p> 
				      	  <p>'.$lan[22].'</p>
				      	  <p>'.$lan[23].'</p>
				      	  <p>'.$lan[24].'</p>
				      	  ...
				      	  <p>'.$lan[27].'</p>   
                       ');
      $zPage->WriteHtml( '<p>'.$lan[28].'</p>');
      $zPage->WriteHtml( '<h2>'.$lan[29].'<a name="RU"></a></h2>');//Россия
      $zPage->WriteHtml( '<p><small> '.$lan[33].'</small></p> ');

      PrintQASummary("Россия",$lan);
    
      $zPage->WriteHtml( '<h2>'.$lan[30].'</h2>');//Заграница
      $zPage->WriteHtml( '<h3>'.$lan[31].'<a name="CIS"></a></h3>');//Ближнее зарубежье
      PrintQASummary("Ближнее Зарубежье",$lan);
      
      //$zPage->WriteHtml( '<h3>'.$lan[32].'<a name="WORLD"></a></h3>'); //Дальнее зарубежье
      //PrintQASummary("Дальнее Зарубежье");
      
      
       $zPage->WriteHtml( '<h2>'.$lan[50].'</h2>');//Cм. также
       $zPage->WriteHtml( '<p> '.$lan[51].' <a href="http://forum.openstreetmap.org/viewtopic.php?id=12233">[*]</a>. </p>');
       $zPage->WriteHtml( '<p>'.$lan[52].' <a href="http://addresses.amdmi3.ru/">[*]</a> </p>
                           <p>'.$lan[53].' <a href="http://wiki.openstreetmap.org/wiki/RU:%D0%92%D0%B0%D0%BB%D0%B8%D0%B4%D0%B0%D1%82%D0%BE%D1%80%D1%8B">[*]</a></p>');       
}	

function PrintQASummary($strGroup,$lan)
{
   global $zPage;
   ...
   $zPage->WriteHtml( '<table width="900px" class="sortable">
   	    ...
   	    <tr style="background: #AFAFAF">
                  <td width="80px"><b>'.$lan[34].'</b></td>
                  <td width="180px"><b>'.$lan[35].'</b></td>
                  <td><b>'.$lan[36].'</b></td>
                  <td><b>'.$lan[37].'</b></td>
                  <td><b>'.$lan[38].'</b></td>
